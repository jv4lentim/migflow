...
class MigrationParserTest < Minitest::Test
  ...
  def parsed
    @parsed ||= Migflow::Parsers::MigrationParser.call(migrations_path: MIGRATIONS_PATH)
  end
  ...
  def test_ignores_non_rb_files
    Dir.mktmpdir do |dir|
      ...
      result = Migflow::Parsers::MigrationParser.call(migrations_path: dir)
      assert_equal 1, result.size
    end
  end

  def test_returns_empty_for_nonexistent_path
    result = Migflow::Parsers::MigrationParser.call(migrations_path: "/nonexistent/path")
    assert_equal [], result
  end
end
